...
class IndexPageTestCase(TestCase):
    ...
    def test_index_page(self):
        ...
        self.assertEqual(response.status_code, 200)# verifier si le code de statut corresponds a 200

class DetailPageTestCase(TestCase):

    # ran before each test.
    def setUp(self):
        impossible = Album.objects.create(title="Transmission Impossible")#creer un nouveau album
        self.album = Album.objects.get(title='Transmission Impossible')# recuperer l'albums 
...
class BookingPageTestCase(TestCase):# test pour les reservation 

    def setUp(self):# la methode setup est executer avant chaque test
        # creation de quelque test
